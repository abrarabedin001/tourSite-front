...
//   }
  ...
  const sendValue= async  (data)=>{
    try {
        let link = toapi+"/"+IdRef.current.value+"/"+PasswordRef.current.value
        console.log(link)
...
{/* onSubmit={handleSubmit} */}
     <h1 className='bold font-bold text-2xl mb-3 text-white'>Customer Sign Up</h1>
      <div  className="max-w-[500px] bg-blue-500 flex flex-wrap space-between flex-col rounded font-bold text-md text-white"  >
        <div key="Id"  className='flex flex-col rounded mt-2 font-bold text-black' >
                    <label htmlFor="Id" className='w-full text-left pl-2'>Id</label>
                    <input type="text" ref={IdRef} id="Id" className="p-2 m-2 w-sm rounded"  placeholder="test"/>
        </div>
        <div key="Password"  className='flex flex-col rounded mt-2 font-bold text-black' >
                    <label htmlFor="Id" className='w-full text-left pl-2'>Password</label>
                    <input ref={PasswordRef} type="text" id="Id" className="p-2 m-2 w-sm rounded"  placeholder="test"/>
        </div>
        <button onClick={handleSubmit} className="bg-blue-200 rounded p-3 m-3 max-w-[200px] font-bold text-lg text-black">Submit</button>
